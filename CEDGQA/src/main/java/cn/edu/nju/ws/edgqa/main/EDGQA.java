...
public class EDGQA extends QASystem {
    ...
    private static JSONArray getDataArray() {
        ...
        }else if (QAArgs.getDataset() == DatasetEnum.Geo) {
            if (QAArgs.isIsTraining()) {
                sparqlArray = new JSONArray(FileUtil.readFileAsString("src/main/resources/datasets//aiwen.json"));
            } else {
                sparqlArray = new JSONArray(FileUtil.readFileAsString("src/main/resources/datasets//zhihu.json"));
            }
        return sparqlArray;
    }
    ...
    public static String getLogFileName() {
        ...
        String logFileName = QAArgs.getDatasetName() + "_" + timeString;
        ...
        for (int quesIdx = quesIdBegin; quesIdx < quesIdEnd; ++quesIdx) { // for each question
            ...
            if (QAArgs.getDataset() == DatasetEnum.LC_QUAD) {
               ...
               // goldenSparql = goldenSparql.replaceAll(" COUNT\\(", " COUNT(DISTINCT ");
                ...
                //Query goldenQuery = QueryFactory.create(goldenSparql, Syntax.syntaxARQ);
                // the golden answers
                goldenAnswer=quesJSONObj.getString("answer");
             }
